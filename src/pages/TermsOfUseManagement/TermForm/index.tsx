...
import { TERM_OF_MANAGEMENT } from '@/constants/routes'
...
const TermForm: React.FC = () => {
  ...
  const onFinish = (values) => {
    ...
    notification.success({
      message: '利用規約を登録しました（バージョン : 2.0.3）',
      className: 'notification-success hasGreenMessage',
      duration: 5,
      icon: <SuccessIcon />,
    })
    console.log('form values: ', values)
    navigate(TERM_OF_MANAGEMENT)
  }

  const onCopyVersion = useCallback(() => {
    navigate('/terms-of-use-management/new-term-of-use-management', {
      ...
      {loading ? (
        ...
        <div className={styles.formContent}>
          ...
          >
            <div className={styles.header}>
              {state?.copyData?.version ? null : (
                <Button type="link" icon={<ArrowLeftIcon />} className={styles.backBtn}>
                  <Link to={TERM_OF_MANAGEMENT}>利用規約管理に戻る</Link>
                </Button>
              )}
              <div
                className={clsx(
              ...
              >
                <p className={styles.title}>
                  {state?.copyData?.version ? '利用規約－新規バージョン登録' : '新規利用規約登録'}
                </p>
            ...
            {isEdit ? (
              ...
              <Form.Item className={styles.actionButtons}>
                <Button htmlType="button" onClick={() => onHandleCancel(false, TERM_OF_MANAGEMENT)}>
                  キャンセル
                </Button>
          ...
          <ModalCancel
            ...
            handleGoBack={() => handleGoBack(TERM_OF_MANAGEMENT)}
